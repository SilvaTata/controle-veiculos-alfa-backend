        ...
        document.addEventListener('DOMContentLoaded', function() {
            setTimeout(() => {
                ...
                if (successMessage) {
                    ...
                    if (bsAlert) { // Verifica se a instância foi criada
                       bsAlert.close();
                    }
                }
                const errorMessage = document.getElementById("errorMessage");
                 if (errorMessage) {
                    const bsErrorAlert = bootstrap.Alert.getOrCreateInstance(errorMessage);
                     if (bsErrorAlert) { // Verifica se a instância foi criada
                        bsErrorAlert.close();
                     }
                }
            }, 5000); // Tempo para fechar alertas iniciais
        });
    </script>
        ...
        <tbody>
            @foreach ($users as $user)
                <tr>
                    ...
                    @endif
                    <td>
                        <a href="{{ route('teste.permissao', $user->id) }}" title="Gerenciar Permissões"> <i class="fa fa-key" aria-hidden="true"></i> </a>
                    </td>
                    <td>
                        @if (auth()->user()->cargo_id == 1)
                            <a href="{{ route('teste.show', $user->id) }}" class="btn btn-info btn-sm" title="Ver Detalhes"><i class="fa fa-eye" aria-hidden="true"></i></a>
                            <a href="{{ route('teste.edit', $user->id) }}" class="btn btn-warning btn-sm" title="Editar Usuário"><i class="fa fa-pencil" aria-hidden="true"></i></a>
                        @else
                            <a href="{{ route('teste.show', $user->id) }}" class="btn btn-info btn-sm" title="Ver Detalhes"><i class="fa fa-eye" aria-hidden="true"></i> Ver</a>
                        @endif
                    </td>
                </tr>
            @endforeach
   ...
   function toggleStatus(userId) {
    ...
    .then(data => {
        if (data.success) {
            ...
            // Usa getOrCreateInstance também para o alerta dinâmico e fecha após timeout
            setTimeout(() => {
                const bsAlertInstance = bootstrap.Alert.getOrCreateInstance(alertDiv);
                if (bsAlertInstance) {
                    bsAlertInstance.close();
                } else if (alertDiv) { // Fallback caso instância não seja criada
                    alertDiv.remove();
                }
            }, 3000); // Tempo para fechar alertas dinâmicos
    ...
    .catch(error => {
        ...
        // Usa getOrCreateInstance também para o alerta de erro dinâmico
        setTimeout(() => {
            const bsAlertInstance = bootstrap.Alert.getOrCreateInstance(alertDiv);
             if (bsAlertInstance) {
                bsAlertInstance.close();
